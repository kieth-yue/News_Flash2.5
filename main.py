...
def scan_once(session_name, turn_count, macro_pushed, stock_pushed, config, prompts):
    ...
    for entry in parse_entries(macro_text):
        ...
        if not valid or (news_time and not (news_after <= news_time <= now_hkt + timedelta(minutes=10))): continue
    ...
    for entry in parse_entries(stock_text):
        ...
        if dedup_key in cache["stock"] or dedup_key in stock_pushed["keys"] or is_duplicate_stock_title(title, code, stock_pushed["titles"]): continue
