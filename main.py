...
def scan_once(session_name, turn_count, macro_pushed, stock_pushed, config, prompts):
    ...
    if turn_count == 1 and macro_prompt:
        ...
        # 1. 先跑板塊
        print("   ➤ 正在掃描【板塊宏觀消息】...")
        p_macro = f"{macro_prompt}{time_info}\n【掃描模式】只輸出「=== 【板塊宏觀消息】 ===」區塊。{format_reminder}{search_command}"
        ...
        # 2. 再跑個股
        print("   ➤ 正在掃描【個股重大利好】...")
        p_stock = f"{stock_prompt}{time_info}\n【掃描模式】只輸出「=== 【個股重大利好】 ===」區塊。{format_reminder}{search_command}"
        ...
        p_stock = (f"{stock_prompt}{time_info}"
                   ...
                   f"若盤中出現突發黑天鵝級宏觀事件，可在最前面輸出「=== 【板塊宏觀消息】 ===」緊急警報。{format_reminder}{search_command}")
